...
function accionProducto(p, sitio = {}) {
  ...
  if (p.url_compra) {
    // Inscripción previa: primero se piden los datos en la web, después se paga.
    // Se desactiva en Configuración del sitio → Venta → "Pedir datos antes de pagar".
    if (venta.pedir_datos !== false) {
      const esFormacion = p.tipo === "curso" || p.tipo === "masterclass";
      return {
        tipo: "inscribirse",
        texto: propio || (esFormacion ? "Inscribirme" : venta.texto_comprar || "Comprar"),
        href: `${p.url}#inscripcion`,
        externo: false,
        nuevaPestana: false,
        pago: p.url_compra,
